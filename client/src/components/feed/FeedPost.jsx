  ...
  <div className="_comment_main _mar_t8">
    <div className="_comment_image">
      <a href="/profile" className="_comment_image_link">
        <img src={txtImg} alt={getFullName(reply.author)} className="_comment_img1" />
      </a>
    </div>
      ...
      <div className="_comment_details">
        <div className="_comment_details_top">
          <div className="_comment_name">
            <a href="/profile"><h4 className="_comment_name_title">{getFullName(reply.author)}</h4></a>
          </div>
        </div>
        ...
        <div className="_comment_reply">
          <div className="_comment_reply_num">
            <ul className="_comment_reply_list">
              <li>
                <button type="button" onClick={() => onToggleLike('reply', reply.id)} style={{ background: 'none', border: 'none', padding: 0, cursor: 'pointer' }}>
                  <span>{reply.isLiked ? 'Unlike' : 'Like'}</span>
                </button>
              </li>
              <li><span className="_time_link">{formatTimeAgo(reply.createdAt)}</span></li>
            </ul>
          </div>
        </div>
      </div>
...
const CommentItem = ({ comment, onToggleLike, onReplyAdded }) => {
  ...
  return (
    <div className="_comment_main">
      <div className="_comment_image">
        <a href="/profile" className="_comment_image_link">
          <img src={txtImg} alt={getFullName(comment.author)} className="_comment_img1" />
        </a>
      </div>
      <div className="_comment_area">
        <div className="_comment_details">
          <div className="_comment_details_top">
            <div className="_comment_name">
              <a href="/profile"><h4 className="_comment_name_title">{getFullName(comment.author)}</h4></a>
            </div>
          </div>
          <div className="_comment_status">
            <p className="_comment_status_text">
          ...
          <div className="_comment_reply">
            <div className="_comment_reply_num">
              <ul className="_comment_reply_list">
                <li>
                  <button type="button" onClick={() => onToggleLike('comment', comment.id)} style={{ background: 'none', border: 'none', padding: 0, cursor: 'pointer' }}>
                    <span>{comment.isLiked ? 'Unlike' : 'Like'}</span>
                  </button>
                </li>
                <li>
                  <button type="button" onClick={() => setShowReplyBox((prev) => !prev)} style={{ background: 'none', border: 'none', padding: 0, cursor: 'pointer' }}>
                    <span>Reply</span>
                  </button>
                </li>
        ...
        {replies.map((reply) => (
          <ReplyItem key={reply.id} reply={reply} onToggleLike={handleReplyLike} />
        ))}
      </div>
    </div>
  );
  ...
  return (
    <div className="_feed_inner_timeline_post_area _b_radious6 _padd_b24 _padd_t24 _mar_b16">
        ...
        <div className="_feed_inner_timeline_post_top">
          <div className="_feed_inner_timeline_post_box">
            <div className="_feed_inner_timeline_post_box_image">
              <img src={postImg} alt={getFullName(post.author)} className="_post_img" />
            </div>
            <div className="_feed_inner_timeline_post_box_txt">
              <h4 className="_feed_inner_timeline_post_box_title">{getFullName(post.author)}</h4>
              <p className="_feed_inner_timeline_post_box_para">
                {formatTimeAgo(post.createdAt)} . <a href="#0">{post.visibility === 'private' ? 'Private' : 'Public'}</a>
              </p>
            </div>
          </div>
          {showDropdown && (
            <div className="_feed_inner_timeline_post_box_dropdown">
              ...
              <div className={`_feed_timeline_dropdown${showDropdown ? ' _timeline_dropdown' : ''}${dropdownOpen ? ' show' : ''}`}>
      ...
      <div className="_timline_comment_main">
        {commentsLoading && (
          <p style={{ fontSize: '13px', color: '#666', padding: '0 24px' }}>Loading comments...</p>
        )}
        {!commentsLoading && comments.length === 0 && (
          <p style={{ fontSize: '13px', color: '#666', padding: '0 24px' }}>No comments yet.</p>
        )}
